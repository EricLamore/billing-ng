...
public class RefundResource {
    ...
    public ResponseEntity<Refund> updateRefund(@PathVariable(value = "id", required = false) final String id, @RequestBody Refund refund)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, refund.getId()))
            .body(result);
    }
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/refunds/{id}", consumes = "application/merge-patch+json")
    public ResponseEntity<Refund> partialUpdateRefund(
        ...
        return ResponseUtil.wrapOrNotFound(result, HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, refund.getId()));
    }
