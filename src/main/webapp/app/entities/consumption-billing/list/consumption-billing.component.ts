...
export class ConsumptionBillingComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.consumptionService
      ...
      .subscribe(
        (res: HttpResponse<IConsumptionBilling[]>) => {
  ...
  trackId(index: number, item: IConsumptionBilling): string {
    return item.id!;
  }

  delete(consumption: IConsumptionBilling): void {
    const modalRef = this.modalService.open(ConsumptionBillingDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IConsumptionBilling[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/consumption-billing'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
